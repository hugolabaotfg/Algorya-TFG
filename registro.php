    ...
    if (strlen($nombre) < 2 || strlen($nombre) > 50) {
        ...
        if ($stmt_check->num_rows > 0) {
            ...
            if ($stmt_insert->execute()) {
                // EMAIL: texto fijo en español, sin t() — se envía al servidor de correo
                $enlace = "https://172.16.200.247/verificar.php?token=" . $token;
                $asunto = "[Accion Requerida] Verifica tu cuenta en Algorya";
                $cuerpo = "Hola " . $nombre . ",\n\n";
                $cuerpo .= "Gracias por registrarte en Algorya. Para activar tu cuenta, haz clic en:\n\n";
                $cuerpo .= $enlace . "\n\n";
                ...
                <div class="card premium-card border-0 rounded-4 shadow-sm">
                    <div class="card-body p-5">
                        ...
                        <div class="text-center mb-4">
                            ...
                            <h2 class="fw-bold mt-2 premium-text">
                                <?= t('registro_titulo') ?>
                            </h2>
                            <p class="premium-muted">
                                <?= t('registro_subtitulo') ?>
                            </p>
                        </div>

                        <?php if ($mensaje != ''): ?>
                            <div class="alert alert-<?= $tipo_alerta ?> alert-dismissible fade show border-0 rounded-3"
                                role="alert">
                                <i
                                    class="bi bi-<?= $tipo_alerta === 'success' ? 'check-circle' : ($tipo_alerta === 'warning' ? 'exclamation-triangle' : 'x-circle') ?>-fill me-2"></i>
                                <?= $mensaje ?>
                        ...
                        <form action="registro.php" method="POST" id="form-registro" novalidate>
                            ...
                            <div class="mb-3">
                                <label for="nombre" class="form-label fw-bold premium-text">
                                    <?= t('registro_nombre') ?>
                                </label>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="password" class="form-control premium-input shadow-none py-2"
                                        id="password" name="password" placeholder="<?= t('registro_pass_hint') ?>"
                                        required>
                                ...
                                <div id="caps-warning" class="text-warning small fw-bold d-none mt-1">
                                    ...
                                    <?= t('registro_caps_aviso') ?>
                                </div>
                            </div>

                            <div class="mb-4">
                                <label for="password_confirm" class="form-label fw-bold premium-text">
                                    <?= t('registro_pass_confirm') ?>
                                </label>
                                <div class="input-group">
                                    <input type="password" class="form-control premium-input shadow-none py-2"
